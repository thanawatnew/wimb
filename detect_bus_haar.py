...
is_ocr=False
is_debug=True
#image_name = 'rgb_20160112_123002'
...
def box(rects, img,img2,img3,img4,path,image_name,file_type):
    count=0
    img_original=img2
    ...
    for x1, y1, x2, y2 in rects:
        ...
        cv2.imwrite(path+'/images_haar/'+image_name+'_result_'+str(count)+file_type, crop_img);
        
	if is_ocr: cv2.imwrite(path+'/ocr/'+image_name+'_ocr_0_'+str(count)+file_type, crop_img_ocr_0);
    if is_ocr: cv2.imwrite(path+'/ocr/'+image_name+'_ocr_1_'+str(count)+file_type, crop_img_ocr_1);
    if is_ocr: call(["tesseract",path+'/ocr/'+image_name+'_ocr_0_'+str(count)+file_type,path+'/ocr_text/'+image_name+'_ocr_0_'+str(count)],stdout=open(os.devnull, 'wb'),stderr=subprocess.STDOUT)
    if is_ocr: call(["tesseract",path+'/ocr/'+image_name+'_ocr_1_'+str(count)+file_type,path+'/ocr_text/'+image_name+'_ocr_1_'+str(count)],stdout=open(os.devnull, 'wb'),stderr=subprocess.STDOUT)
    
    if is_debug: print 'save to '+path+'/images_haar_result/'+image_name+'_result'+file_type
    cv2.imwrite(path+'/images_haar_result/'+image_name+'_result'+file_type, img);
...
def detect_bus_haar(path,image_name,file_type,is_ocr,cascade):
	 ...
	 img2 = cv2.imread(path+'/images/'+image_name+file_type,cv2.IMREAD_COLOR)
	 if is_ocr: img3 = cv2.imread(path+'/ocr/'+image_name+'_preprocess_0'+file_type,cv2.IMREAD_COLOR)
	 if is_ocr: img4 = cv2.imread(path+'/ocr/'+image_name+'_preprocess_1'+file_type,cv2.IMREAD_COLOR)
	 #print img
	 rects, img = detect(img,cascade)
	 if is_ocr: box(rects, img,img2,img3,img4,path,image_name,file_type)
	 else: box(rects, img,img2,None,None,path,image_name,file_type)
	 return rects
	 #cv2.imshow("frame", img)
def detect_bus_haar_group(path):
	...
	for file_name_with_extension in os.listdir(path+'/images/')[count:]:
	 ...
	 image_name,file_type=file_name_with_extension.split('.')[0],'.'+file_name_with_extension.split('.')[1]
	 detect_bus_haar(path,image_name,file_type,False,cascade)
